...
def train(
    args, train_dataset, model: BertForMlmWithClassification, tokenizer: BertTokenizer
) -> Tuple[int, float]:
...
def evaluate(
    args, model: BertForMlmWithClassification, tokenizer: BertTokenizer, prefix=""
) -> Dict:
    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        cls_preds = torch.softmax(cls_scores, dim=1).detach().cpu().numpy().argmax(axis=1)
        class_labels = class_labels.detach().cpu().numpy()

    eval_loss = eval_loss / nb_eval_steps
    result = {
        "loss": eval_loss,
        "macro_f1": f1_score(class_labels, cls_preds, average="macro"),
        "micro_f1": f1_score(class_labels, cls_preds, average="micro"),
    }
    if args.mlm:
